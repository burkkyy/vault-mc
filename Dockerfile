...
FROM node:23.2.0-alpine3.20 AS base-node
...
COPY --from=api-build-stage /usr/src/api/package*.json ./
RUN npm install && npm cache clean --force --loglevel=error

COPY --from=api-build-stage /usr/src/api/dist ./dist/
COPY --from=web-build-stage /usr/src/web/dist ./dist/web/
...
COPY --from=api-build-stage  /usr/src/api/bin/boot-app.sh ./bin/
